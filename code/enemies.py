...
class Enemy(pygame.sprite.Sprite, ABC):
    def __init__(self, pos, groups, collision_sprites, attack_sprites, player_sprites, player, frames, attack_data):
        ...
        self.timers = {
            "attack_cooldown": Timer(1500, sustained=True),
        ...
        self.damage_sprites = player_sprites
...
class SpringBasic(Enemy):
    def __init__(self, pos, groups, collision_sprites, attack_sprites, player_sprites, player, frames, attack_data):
        super().__init__(pos, groups, collision_sprites, attack_sprites, player_sprites, player, frames, attack_data)
    ...
    def move(self, dt):
        self.velocity.x = 0
        if self.player_found and not self.can_attack:
            self.direction = 1 if self.player_pos[0] > self.hitbox_rect.center[0] else -1

            if (self.direction > 0 and self.collisions["right_edge"]) or (self.direction < 0 and self.collisions["left_edge"]):
    ...
    def attack(self):
        ...
        if 'melee' in self.state:
            if self.frame_index >= len(self.frames[self.state]):
                ...
                if random.randint(0,3) == 1 and self.attack_stage == 1:
                    self.attack_stage += 1
                    self.attack_combo = True
                else:
                    self.timers['attack_cooldown'].activate()
                    self.attack_combo = False
                    self.attack_stage = random.choice([1, 1, 1, 2])
